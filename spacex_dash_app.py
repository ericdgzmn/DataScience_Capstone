...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                html.Div([
                                    html.Div([
                                        html.H2('Select a launch site here:', style={'margin-right': '2em'}),
...
def pie(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        fig = px.pie(df, values='class', names='Launch Site', title='Success Counts for Launch Sites in US')
    else:
...
def build_scatter(site_dropdown,payload):
    ...
    if site_dropdown == 'ALL':
        ...
        filtered_dfb= filtered_dfa[filtered_dfa['Launch Site'] == site_dropdown]
        fig = px.scatter(filtered_dfb,x="Payload Mass (kg)", y="class", color="Booster Version Category", title='Payload vs. Outcome for ' + site_dropdown)
        return fig
# Run the app
